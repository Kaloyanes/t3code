...
it.layer(NodeSqliteClient.layer({ filename: ":memory:" }))(
  ...
  (it) => {
    it.effect("fills both branches of migration 54 for an upstream database", () =>
      Effect.gen(function* () {
        const sql = yield* SqlClient.SqlClient;
        yield* runMigrations({ toMigrationInclusive: 53 });
        yield* migrateAutoSettleDisabledAt;
        const issueTables = yield* sql<{ readonly name: string }>`
          SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'projection_issue_links'
        `;
        assert.deepEqual(issueTables, [{ name: "projection_issue_links" }]);
